...
test("worker registration commands honor the shared state lock", async () => {
  ...
  const holder = Bun.spawn({
    cmd: ["flock", join(f.config, "state.lock"), "-c", `touch ${marker}; sleep 3`],
    stdout: "ignore",
    stderr: "pipe",
  });
  try {
