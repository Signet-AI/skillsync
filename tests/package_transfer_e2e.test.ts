...
async function packageFixture(prefix: string) {
  ...
  const payload = Buffer.from("stable package bytes");
...
test("package transfer stage publishes the strict external artifact layout", async () => {
  ...
  try {
    ...
    const payload = Buffer.from("external output bytes");
...
test("package transfer inspect accepts a strict local package tree", async () => {
  ...
  try {
    ...
    expect(result.exitCode, dec.decode(result.stderr)).toBe(0);
    expect(JSON.parse(dec.decode(result.stdout)).format).toBe("skillsync-package-transfer");
  } finally { await rm(root, { recursive: true, force: true }); }
});
