    ...
    loop {
        terminal.draw(|frame| {
            ...
            if detail && !inventory.packages.is_empty() {
                ...
                let text = format!("{}\n\nPath: {}\nProvenance: {}\nRelationships: subscriptions {}, publications {}, adoptions {}\nHarness health: {}\nCapabilities:\n{}\nDiscovery boundaries:\n{}\n\nEnter/Esc: back", p.name, p.path, source, p.relationship.subscriptions.len(), p.relationship.publications.len(), p.relationship.local_adoptions.len(), inventory.harness_links.iter().map(|h| h.status.as_str()).collect::<Vec<_>>().join(", "), inventory.capabilities.typed.capabilities.iter().map(|c| format!("{}: {:?}/{:?} — {}", c.id, c.support, c.verification, c.reason)).collect::<Vec<_>>().join("\n"), inventory.capabilities.typed.boundaries.iter().map(|b| format!("{}: {:?} — {}", b.id, b.status, b.reason)).collect::<Vec<_>>().join("\n"));
